...
class MambaCache:
    ...
    def __init__(
        ...
        self.conv_states = {
            i: torch.zeros(batch_size, intermediate_size,
                           conv_kernel_size, device=device, dtype=dtype)
            for i in range(config.num_hidden_layers)
        }
        self.ssm_states = {
            i: torch.zeros(batch_size, intermediate_size,
                           ssm_state_size, device=device, dtype=dtype)
            for i in range(config.num_hidden_layers)
        }
...
class MambaMixer(nn.Module):
    ...
    def __init__(self, config: MambaConfig, layer_idx: int):
        ...
        A = torch.arange(1, self.ssm_state_size + 1,
    ...
    def cuda_kernels_forward(self, hidden_states: torch.Tensor, cache_params: Optional[MambaCache] = None):
        ...
        if self.training and cache_params is None:  # Doesn't support outputting the states -> used for training
            ...
            time_step, B, C = torch.split(
                ssm_parameters, [self.time_step_rank,
                                 self.ssm_state_size, self.ssm_state_size], dim=-1
            )
            discrete_time_step = self.dt_proj.weight @ time_step.transpose(
                1, 2)
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, MambaMixer):
            ...
            if self.config.time_step_init_scheme == "constant":
                ...
                nn.init.uniform_(module.dt_proj.weight, -
...
class MambaSequenceClassificationOutput(ModelOutput):
    """
    Base class for a MAMBA based language model applied to a Sequence Classification task's outputs.
        ...
        for k in state_dict:
            if "embedding." in k:
                state_dict[k.replace(
                    "embedding.", "embeddings.")] = state_dict.pop(k)
                break
...
class MambaForCausalLM(MambaPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,  # for now we need this for generation
        ...
        logits = self.lm_head(hidden_states.to(
            self.lm_head.weight.dtype)).float()
...
@add_start_docstrings(
    """
    THE MAMBA Model transformer with a sequence classification/regression head on top
    (a linear layer applied to the last output).
    """,
    MAMBA_START_DOCSTRING,
)
class MambaForSequenceClassification(MambaPreTrainedModel):
    ...
    def forward(
        ...
        use_cache: Optional[bool] = None,
        **kwargs
    ) -> Union[Tuple[torch.Tensor], MambaSequenceClassificationOutput]:
        r"""
        ...
        logits = self.classifier(
            hidden_states[:, :, -1].to(self.classifier.weight.dtype)
        ).float()
